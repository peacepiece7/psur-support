<script setup lang="ts">
  import type { NewRegSprtClubFacility } from '~/types/apis/registered-sports-club'

  type Props = {
    isOpen: boolean
    options: NewRegSprtClubFacility[]
  ...
  const emit = defineEmits<{
    ...
    (e: 'close'): void
    (e: 'submit', value: NewRegSprtClubFacility[]): void
  }>()
...
<template>
  <Dialog v-model="dialog" max-width="560">
    <template #title>사용시설 검색</template>
      ...
      <v-list lines="two" density="compact">
        ...
        >
          ...
          <v-list-item-subtitle>
            {{ opt.address }}
            <template v-if="opt.note"> · {{ opt.note }}</template>
          </v-list-item-subtitle>
        </v-list-item>
      </v-list>
    </template>
